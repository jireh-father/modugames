...
|------|----------|--------|----------------|-----------|------|------|
| **권총** | 400px | 2 | 125px | 40px | 없음 | 범용, 탄창 10발 |
| **활** | 500px | 3 | 0 (무음) | 25px | 1마리 관통 | 스텔스, X 마커 조이스틱 조준 |
| **저격총** | 무제한 | 5 | 200px | 75px | 완전 관통 | 최강 화력, 10발 탄창 |
| **기관총** | 350px | 1 | 175px | 30px | 없음 | 탄띠 자동 연사 |
| **석궁** | 450px | 4 | 40px (미약) | 30px | 1마리 관통 | 준스텔스 |
...
#### 권총
- 3열 레이아웃: 슬라이드 | 방아쇠 | 탄창
- 슬라이드 아래 드래그 → 장전 (약실에 탄 넣기)
- 방아쇠: 좌우 드래그=조준, 탭=발사
- 탄창: 아래 드래그 → 탄창 분리 → 재장전 팝업
- 재장전 팝업: 총알통→탄창 드래그, 탄창→총 드래그로 삽입

#### 기관총 (v12 재설계)
- 3열 레이아웃: 탄띠 | 기관총 | 여분탄
- **탄띠에 탄약 있으면 자동 연사** (수동 트리거 없음)
- 여분 총알을 왼쪽 탄띠로 드래그하면 장전 (한번에 최대 10발)
- 과열/코킹 시스템 제거
- 가운데: 좌우 드래그로 조준

#### 활 (v12 재설계)
- 2열 레이아웃: 활 (60%) | 조이스틱 (40%)
- 활 영역 터치 → 자동 화살 장전 + 시위 당기기 시작
- 아래로 드래그할수록 파워 증가, 놓으면 발사
- 오른쪽 원형 조이스틱으로 타겟 포인트 이동
- 필드에 **X 마커**로 타겟 위치 표시 (직선 조준선 없음)
- 화살통에서 수동 장전 불필요 (자동 장전)

#### 저격총 (v12 재설계)
- 3열 레이아웃: 볼트 | 방아쇠 | 탄창
- 볼트: 위 드래그=열기(탄피 배출), 아래 드래그=닫기(탄창에서 장전)
- 방아쇠: 좌우 드래그=조준, 탭=발사, 발사 후 자동 볼트 열림
- 탄창: **권총식 10칸 탄창** 표시
- 아래 드래그 → 탄창 분리 → 재장전 팝업 (권총과 동일 구조)
- 재장전 팝업: 총알→탄창 드래그, 탄창→총 드래그로 삽입
- 스코프 기능 제거
...
| **지뢰** | 좀비 접근 시 폭발 (반경 60px, 5뎀) | 폭발 시 100px | 8 |
| **화염병** | 화염 지역 생성 (반경 50px, 2뎀/초, 3초) | 50px 3초 | 8 |
| **폭탄** | 즉시 폭발 (반경 80px, 5뎀) | 125px | 2 |
| **장난감** | 소리 유인 (75px, 5초) | 75px 5초 | 10 |
| **폭죽** | 소리 유인 (150px, 3초) + 종료 시 폭발 | 150px 3초 | 8 |
| **라디오** | 소리 유인 (100px, 10초) | 100px 10초 | 6 |
...
- 소리 범위: 150px, 지속: 3초
- 넓은 영역 유인
- 종료 시 **폭발** (반경 80px, 5뎀) + 폭발 소리(125px)
...
- **소리 유인**: 소리 범위 원형 (toy: 75, firecracker: 150, radio: 100)
...
### 10.1 구성
- 4개 벽 구간, 각각 폭 135px (틈새 없이 밀접 배치)
