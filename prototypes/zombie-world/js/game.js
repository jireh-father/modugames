...
// 소리 크기 등급 (4x 확대): 0 무음 | 120 미미 | 200 작음 | 320 보통 | 480 큼 | 720 매우큼 | 1000 폭발급
...
export const state = {
  ...
  player: {
    ...
    targetBuilding: null, // 건물 탭 시 자동 진입 대상
...
export function resetGame() {
  ...
  state.player = {
    ...
    targetTower: -1, targetEdgeDir: null, targetBuilding: null, moveNoiseRange: 160, moveNoiseTimer: 0, shoeType: null, shoeTimer: 0,
  };
  state.hunger = 100;
  state.fatigue = 100;
...
export function emitSound(x, y, range, duration = 1.0, type = 'generic', target = null) {
  if (range <= 0) return;
  // 날씨 소리 배율
  const weatherSoundMul = state.currentWeather === 'rain' ? 0.5
                        : state.currentWeather === 'storm' ? 1.3
                        : 1.0;
  // 밤에는 소리가 40% 더 멀리 퍼짐
  const actualRange = (state.isNight ? range * 1.4 : range) * weatherSoundMul;
