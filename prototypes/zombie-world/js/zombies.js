...
import { W, state, WALL_Y, TOWER_Y, WEAPON_PROFILES, emitSound } from './game.js?v=311';
import { getWallY, getWallSegments } from './wall.js?v=311';
import { playZombieHit, playZombieDeath, playWallHit, playWallBreak, playTowerHit,
         ...
         playFreezeApply, playPoisonApply } from './audio.js?v=311';
...
function updateZombies(dt) {
  ...
  for (let i = state.zombies.length - 1; i >= 0; i--) {
    ...
    const nightMul = state.isNight ? 1.2 : 1.0;
...
function drawZombies(ctx) {
  for (const z of state.zombies) {
    if (!z.alive) continue;
    // 화면 밖 좀비 렌더링 스킵
    if (z.x < -50 || z.x > W + 50 || z.y < -50 || z.y > 700) continue;
